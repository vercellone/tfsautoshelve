    ...
    public class TestUtils
    {
        ...
        public static void WriteEmbeddedResourceToFile(Assembly assembly, string embeddedResourceName, string fileName)
        {
            ...
            if (fileContents == null)
                throw new ApplicationException("Failed to get embedded resource '" + embeddedResourceName +
        ...
        public static void WriteEmbeddedResourceToBinaryFile(Assembly assembly, string embeddedResourceName,
            string fileName)
        {
            // Get file contents
            Stream stream = assembly.GetManifestResourceStream(embeddedResourceName);
            if (stream == null)
                throw new InvalidOperationException("Failed to get embedded resource '" + embeddedResourceName +
                                                    "' from assembly '" + assembly.FullName);
        ...
        /// <returns></returns>
        private static string GetNewFileOrDirectoryNameWithoutCreatingAnything(string directory, string baseFileName,
            string extension)
        {
        ...
        public void CreateEmptySolution(string directory, string solutionName)
        {
            ...
            Assert.AreEqual(solutionName + ".sln", Path.GetFileName(dte.Solution.FileName),
                "Newly created solution has wrong Filename");
        }
        ...
        /// <returns></returns>
        public ProjectItem AddNewItemFromVsTemplate(ProjectItems parent, string templateName, string language,
            string name)
        {
            if (parent == null)
