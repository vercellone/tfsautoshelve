...
namespace VsExt.AutoShelve_IntegrationTests
{
    ...
    internal class DialogBoxPurger : IDisposable
    {
        ...
        internal bool WaitForDialogThreadToTerminate(int numberOfMillisecondsToWait)
        {
            ...
            var signaled = _threadDone.WaitOne(numberOfMillisecondsToWait, false);

            // Kill the thread since a timeout occured.
            if (signaled) return _dialogBoxCloseResult;
            lock (Mutex)
            {
        ...
        private void HandleDialogBoxes()
        {
            ...
            var hwnds = new IntPtr[_numberOfDialogsToWaitFor];
            var dialogBoxCloseResults = new bool[_numberOfDialogsToWaitFor];
            ...
            try
            {
                ...
                while (stayInLoop)
                {
                    ...
                    if (hwnds[hwndIndex] != IntPtr.Zero)
                    {
                        ...
                        if (!windowClassName.ToString().Contains("#32770")) continue;
            ...
            finally
            {
                ...
                foreach (var hwnd in hwnds.Where(hwnd => hwnd != IntPtr.Zero))
                {
                ...
                {
                    // Be optimistic.
                    _dialogBoxCloseResult = dialogBoxCloseResults.All(t => t);
                    _threadDone.Set();
                }
            }
        }
